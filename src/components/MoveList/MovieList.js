...
import { allMovieData, fetchAllMovie, searchData } from "../../features/counter/movieCaller";
...
function MovieList() {
  ...
  useEffect(()=>{
    const fetch = async ()=>{
      const response = await movieApi.get(`http://www.omdbapi.com/?apikey=15ae9fea&s=${search?search:'harry'}&page=${page+''}`)
      .catch((err)=>{
        console.log(err)
      })
